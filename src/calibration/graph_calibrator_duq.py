...
from calibration.attention_ts import CalibAttentionLayer
...
DEVICE='cuda'
def fit_calibration(temp_model, eval, train_dataloader, test_dataloader, patience = 100):
    """
    Train calibrator
    """    
    vlss_mn = float('Inf')
    ...
    with torch.no_grad():
        for batch in train_dataloader:
                ...
                logits = temp_model.model(batch)
                train_logits_list.append(logits)
                train_labels_list.append(batch.y)
        train_logits = torch.cat(train_logits_list).to(DEVICE)
        train_labels = torch.cat(train_labels_list).to(DEVICE)
        for batch in test_dataloader:
                batch = batch.to(DEVICE)
                logits = temp_model.model(batch)
                test_logits_list.append(logits)
                test_labels_list.append(batch.y)
    ...
    for epoch in range(2000):
        ...
        calibrated = eval(train_logits)
        loss = F.cross_entropy(calibrated,train_labels)
        ...
        with torch.no_grad():
            temp_model.eval()
            calibrated = eval(test_logits)
            val_loss = F.cross_entropy(calibrated, test_labels)
            # dist_reg = intra_distance_loss(calibrated[train_mask], labels[train_mask])
...
class TS(nn.Module):
    ...
    def forward(self,batch):
        logits,_ = self.model(batch,anchors=self.anchors, n_anchors=self.n_anchors,return_std=True)
    ...
    def fit(self, train_dataloader, test_dataloader, wdecay):
        self.to(device)
        def eval(logits):
            temperature = self.temperature_scale(logits)
        ...
        fit_calibration(self, eval, train_dataloader, test_dataloader )
        return self
...
class ETS(nn.Module):
    ...
    def fit(self, train_dataloader, test_dataloader, wdecay):
        self.to(device)
        self.temp_model.fit(train_dataloader, test_dataloader, wdecay)
        torch.cuda.empty_cache()

        train_logits_list = []
        train_labels_list = []
        self.temp_model.model.eval()
        with torch.no_grad():
            for batch in train_dataloader:
                    batch = batch.to(DEVICE)
                    logits,_ = self.temp_model.model(batch,anchors=self.anchors, n_anchors=self.n_anchors,return_std=True)
                    # logits = self.temp_model.model(batch)
                    train_logits_list.append(logits)
                    train_labels_list.append(batch.y)
            train_logits = torch.cat(train_logits_list).to(DEVICE)
            train_labels = torch.cat(train_labels_list).to(DEVICE)
        # train_logits = self.model(data.x, data.edge_index)[train_mask]
        # train_label = data.y[train_mask]
        # pdb.set_trace()
        # if train_labels.shape[1] == 1:
        #     # pass
        #     train_logits = torch.sigmoid(train_logits)
        #     one_hot = train_labels.long()
        #     # one_hot.scatter_(1, train_labels.int(), 1)
        # else:
        one_hot = torch.nn.functional.one_hot(train_labels.long(), num_classes= train_labels.max().int() + 1).long() 
        temp = self.temp_model.temperature.cpu().detach().numpy()
        w = self.ensemble_scaling(train_logits.cpu().detach().numpy(), one_hot.cpu().detach().numpy(), temp)
...
class IRM(nn.Module):
    ...
    def fit(self, train_dataloader, test_dataloader, wdecay):
        ...
        train_logits = torch.cat(train_logits_list).to(DEVICE)
        train_labels = torch.cat(train_labels_list).to(DEVICE)
        train_p = torch.softmax(train_logits, -1).cpu().numpy()
        train_y = F.one_hot(
            train_labels, train_p.shape[-1]).cpu().numpy()
...
class Dirichlet(nn.Module):
    ...
    def fit(
            self, train_dataloader, test_dataloader, wdecay, patience=100):
        self.to(device)
        ...
        for epoch in range(2000):
            ...
            self.model.eval()
            optimizer.zero_grad()
            calibrated = self.calibrate(train_logits)
            loss = F.cross_entropy(
                calibrated,train_labels 
            ...
            self.eval()
            with torch.no_grad():
                calibrated = self.calibrate(test_logits)
                val_loss = F.cross_entropy(
                    calibrated,test_labels 
                ) + self.odir_loss(wdecay, wdecay)
                if val_loss <= vlss_mn:
                    state_dict_early_model = copy.deepcopy(parameters)
                    vlss_mn = np.min((val_loss.cpu().numpy(), vlss_mn))
...
class SplineCalib(nn.Module):
    ...
    def fit(self, train_dataloader, test_dataloader, wdecay):
        ...
        # logits = self.model(batch)[train_mask]
        scores, preds = torch.softmax(train_logits, -1).max(-1)
        corrects = torch.eq(preds, train_labels)
        ...
        del train_logits, scores, preds, corrects, sort_idx
...
class OrderInvariantCalib(nn.Module):
    ...
    def forward(self, batch):
        return self.calibrate(self.model(batch))
    ...
    def fit(self, train_dataloader, test_dataloader, wdecay):
        ...
        fit_calibration(
            self, self.calibrate, train_dataloader, test_dataloader)
        return self